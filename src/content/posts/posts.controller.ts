...
export class PostsController {
  ...
  @UseGuards(JwtAuthGuard)
  @Post('api/posts/createpost2')
  @UseInterceptors(FileInterceptor('postContent'))
  async createPostV3new2(@UploadedFile() file: Express.Multer.File, @Body() body, @Headers() headers): Promise<CreatePostResponse> {
    this.logger.log("createPost >>> start");
    console.log('>>>>>>>>>> BODY <<<<<<<<<<', JSON.stringify(body))
    var arrtag = [];
    ...
    var data = await this.postContentService.createNewPostV32(file, body, headers);
    var postID = data.data.postID;
