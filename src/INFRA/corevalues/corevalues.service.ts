...
export class CorevaluesService {
    constructor(
        @InjectModel(Corevalues.name) private readonly corevaluesModel: Model<CorevaluesDocument>,
      ) {}
     
      async create(CreateCorevaluesDto: CreateCorevaluesDto): Promise<Corevalues> {
        const createCorevaluesDto = await this.corevaluesModel.create(CreateCorevaluesDto);
