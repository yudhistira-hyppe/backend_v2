...
export class FsChunksService {
    constructor(
        @InjectModel(Fschunks.name) private readonly fschunksModel: Model<FschunksDocument>,
      ) {}
     
      async create(CreateFschunksDto: CreateFschunksDto): Promise<Fschunks> {
        const createFschunksDto = await this.fschunksModel.create(CreateFschunksDto);
