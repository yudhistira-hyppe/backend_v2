...
export class AwsController {
    ...
    async uploadcomparing(@UploadedFiles() files: { avatar?: Express.Multer.File[], background?: Express.Multer.File[] }, @Res() response) {
        const bitmap1 = await fs.readFileSync('./upload/' + files.avatar[0].filename, { encoding: 'base64' });
        const bitmap2 = await fs.readFileSync('./upload/' + files.background[0].filename, { encoding: 'base64' });
