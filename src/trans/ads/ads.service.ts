...
export class AdsService {
    ...
    async listusevoucher(userid: ObjectID, startdate: string, enddate: string, page: number, limit: number): Promise<Ads[]> {
        try {
            var currentdate = new Date(new Date(enddate).setDate(new Date(enddate).getDate() + 1));
        ...
        else if (userid !== undefined && startdate === undefined && enddate === undefined) {
            let query = await this.adsModel.aggregate([
    ...
    async listusevouchercount(userid: ObjectID, startdate: string, enddate: string): Promise<Ads[]> {
        try {
            var currentdate = new Date(new Date(enddate).setDate(new Date(enddate).getDate() + 1));
