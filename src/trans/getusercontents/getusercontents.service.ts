...
export class GetusercontentsService {
  ...
  async findalldatakontenmonetesbuy(iduserbuy: Types.ObjectId, email: string, buy: boolean, monetize: boolean, postType: string, lastmonetize: boolean, skip: number, limit: number): Promise<object> {
    const posts = await this.postsService.findpost();
    const video = await this.mediavideosService.findvideo();
    const pict = await this.mediapictsService.findpict();
    const mediaprofil = await this.mediaprofilepictsService.findmediaprofil();
    ...
    if (buy === true && monetize === false && postType === undefined && lastmonetize === false) {
      const query = await this.getusercontentsModel.aggregate([
        {
          $addFields: {
            ubasic_id: '$userProfile.$id',
    ...
    else if (buy === false && monetize === true && postType === undefined && lastmonetize === false) {
      const query = await this.getusercontentsModel.aggregate([
    ...
    else if (buy === true && monetize === false && postType !== undefined && lastmonetize === false) {
      const query = await this.getusercontentsModel.aggregate([
        {
          $addFields: {
            ubasic_id: '$userProfile.$id',
    ...
    else if (buy === false && monetize === false && postType === undefined && lastmonetize === true) {
      const query = await this.getusercontentsModel.aggregate([
